...
function activate(context) {
    ...
    const exportCommand = vscode.commands.registerCommand('dev-cheatsheet.export', async () => {
        try {
            ...
            if (fs.existsSync(targetFile)) {
                const choice = await vscode.window.showWarningMessage('cheats.json already exists in the current folder. Overwrite?', 'Yes', 'No');
                if (choice !== 'Yes') {
                    return;
                }
            }
            ...
            const openChoice = await vscode.window.showInformationMessage('Export successful! Open the file?', 'Open', 'Close');
            if (openChoice === 'Open') {
    ...
    const importCommand = vscode.commands.registerCommand('dev-cheatsheet.import', async () => {
        try {
            ...
            vscode.window.showInformationMessage(`Successfully imported ${importedCheatsheets.length} cheatsheet(s) from ${path.basename(selectedFile)}`);
            // Suggest to open the viewer
            const openChoice = await vscode.window.showInformationMessage('Import successful! Open cheatsheets viewer?', 'Open', 'Close');
            if (openChoice === 'Open') {
...
function loadCheatsheets(context) {
    ...
    const config = vscode.workspace.getConfiguration('dev-cheatsheet');
    const importedFilePath = config.get('importedFilePath');
